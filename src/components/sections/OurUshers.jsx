...
const OurUshers = () => {
  ...
  // Create a default usher image as data URL (base64 encoded black rectangle)
  const defaultUsherImage = "data:image/svg+xml,%3Csvg width='300' height='400' xmlns='http://www.w3.org/2000/svg'%3E%3Crect width='300' height='400' fill='%23000000'/%3E%3Ctext x='150' y='200' text-anchor='middle' fill='%23ffffff' font-size='18' font-family='Arial'%3EPROFESSIONAL%3C/text%3E%3Ctext x='150' y='220' text-anchor='middle' fill='%23ffffff' font-size='18' font-family='Arial'%3EUSHER%3C/text%3E%3C/svg%3E"
  ...
  const prevSlide = () => {
    setCurrentSlide((prev) => (prev - 1 + Math.ceil(ushers.length / 3)) % Math.ceil(ushers.length / 3))
  }
        ...
        <div className="text-center mb-20">
          ...
          >
            ...
            <p className="text-xl text-gray-300 max-w-3xl mx-auto leading-relaxed font-light tracking-wide">
              Meet our team of trusted, verified professionals ready to make your event seamless.
            </p>
          </motion.div>
        </div>
          ...
          <>
            ...
            <div className="relative mb-16">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
                {ushers.slice(currentSlide * 3, (currentSlide + 1) * 3).map((usher, index) => (
                  <motion.div
                    ...
                    <div className="bg-primary-black border border-gray-700 hover:border-secondary-white/30 p-8 transition-all duration-500 group-hover:scale-105">
                      ...
                      <div className="text-center">
                        ...
                        <p className="text-gray-400 mb-6 line-clamp-3 font-light leading-relaxed">
                          {usher.profile?.bio || 'Professional usher with extensive experience in event management.'}
                        </p>

                        {usher.profile?.skills && usher.profile.skills.length > 0 && (
                          <div className="flex flex-wrap justify-center gap-3 mb-6">
                            {usher.profile.skills.slice(0, 3).map((skill, skillIndex) => (
                              <span
                                key={skillIndex}
                        ...
                        <div className="flex items-center justify-center">
                          <div className={`w-3 h-3 mr-3 ${
                            usher.profile?.availability ? 'bg-green-400' : 'bg-gray-500'
                          }`}></div>
                          <span className="text-sm text-gray-300 font-light tracking-wide">
                            {usher.profile?.availability ? 'AVAILABLE' : 'BUSY'}
                          </span>
                        </div>
                      </div>
                    </div>
